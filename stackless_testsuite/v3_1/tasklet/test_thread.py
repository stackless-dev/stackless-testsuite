...
def GetRemoteTasklet(callable_, args):
    """Get a non-scheduled tasklet on a remote thread"""
    tasklets, thread = GetRemoteTasklets([lambda:callable_(*args)])
...
@unittest.skipUnless(withThreads, "requires thread support")
class TestRebind(StacklessTestCase):
    def create_remote_tasklet(self, nontrivial=False, job=None):
        if nontrivial:
            try:
                stackless.test_cstate
            except AttributeError:
                self.skipTest("This implementation does not support non trivial stack layouts")
        result = []
        e1 = threading.Event()
        e2 = threading.Event()
...
class SchedulingBindThreadTests(RemoteTaskletTests):
    ...
    def test_bind_to_other_tid(self):
        ...
        theThread, t = self.create_thread_task()
        with theThread:
            otherThread, t2 = self.create_thread_task()  # @UnusedVariable
            with otherThread:
                self.assertEqual(t.thread_id, theThread.ident)
