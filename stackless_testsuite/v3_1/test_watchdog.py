...
# Copyright (c) 2014 by Christian Tismer
# Copyright (c) 2014 by Kristjan Valur Jónsson
...
def is_soft():
    ...
    return softswitch and not _in_psyco()
...
def runtask6(name):
    ...
    for ii in xrange(1000):  # @UnusedVariable
...
class TestWatchdog(StacklessTestCase):
    ...
    def test_send_recv(self):
        ...
        server_task = server("server", chan)  # @UnusedVariable

        scheduler = SimpleScheduler(100, self.softSchedule)

        [stackless.tasklet(runtask4)(name, chan)
        ...
        # Kill server
        self.assertRaises(
            StopIteration, lambda: chan.send_exception(StopIteration))
    ...
    def test_run_return(self):
        ...
        pickle.loads(self.get_pickled_tasklet())
...
class TestWatchdogSoft(TestWatchdog):
    ...
    def __init__(self, *args):
        self.chans = [stackless.channel() for i in xrange(3)]  # @UnusedVariable
        # for c in self.chans:
        #    c.preference = 0
...
class TestNewWatchdog(StacklessTestCase):
    ...
    def test_main_exiting(self):
        ...
        stackless.schedule()
        # The other dude won't exit its run until we are no longer runnable.
        self.assertEqual(self.done, 1)
    ...
    def _test_watchdog_priority(self, soft):
        ...
        def runner_func(recursive, start):
            ...
            with stackless.atomic():
                stackless.run(
                    2, soft=soft, totaltimeout=True, ignore_nesting=True)
                a = self.awoken
                self.awoken += 1
            if recursive == start:
        ...
        t1 = stackless.tasklet(task)()
        t2 = stackless.tasklet(task)()
        stackless.tasklet(runner_func)(3, 3)
        stackless.run()
        self.assertEqual(self.done, 2)
